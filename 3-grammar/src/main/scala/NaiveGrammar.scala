...
object NaiveGrammar extends Combinators {
  ...
  def parseRHS(ruleRHS: RuleRHS, grammar: Grammar): Parser[List[Tree]] = 
    ruleRHS match {
      case nt:Nonterminal => parseNonterminal(nt, grammar) ^^ { t => List(t) }
  ...
  def simplifyAE(syntaxTree: Tree): Tree = syntaxTree match {
    case Branch('exp, List(le)) => simplifyAE(le)

    case Branch(symbol, ln) => Branch(symbol, ln.map(simplifyAE).filter(
      x => x match {
        case Leaf('keyword, _) => false
        case otherwise => true
      }
    ))
