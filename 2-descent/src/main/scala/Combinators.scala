...
trait Combinators extends AE {
   ...
   * parse3(" sum    of     1   and 1 ") == Add(Num(1), Num(1))
   */
  def parseString2(stringToMatch: String):Parser[String] = code => {
  ...
  def parseSpace: Parser[String] = parseString2(" ")



  def parseMin0Spaces: Parser[String] = zeroOrMore(parseSpace) ^^ (list => {
    if(list.isEmpty) ""
    else list.mkString("")
  })

  def parseMin1Spaces: Parser[String] = oneOrMore(parseSpace) ^^ (list => list.mkString("")) 

  def exp3: Parser[Exp] = parseMin0Spaces ~> (add3 | mul3 | num3)

  def add3: Parser[Exp] =
    (parseMin0Spaces ~> sumOf3 ~> parseMin1Spaces ~> exp3 <~ parseMin1Spaces <~ and3) ~ exp3 ^^ {
      case (lhs, rhs) => Add(lhs, rhs)
    }

  def mul3: Parser[Exp] =
    (parseMin0Spaces ~> productOf3 ~> parseMin1Spaces ~> exp3 <~ parseMin1Spaces <~ and3) ~ exp3 ^^ {
      case (lhs, rhs) => Mul(lhs, rhs)
    }

  def productOf3: Parser[String] = parseString2("product") ~> parseMin1Spaces ~> parseString2("of") 

  def sumOf3: Parser[String] = parseString2("sum") ~> parseMin1Spaces ~> parseString2("of") 

  def and3: Parser[String] = parseString2("and")

  def num3: Parser[Exp] = parseNum

  def parse3(code: String): Exp = {
    (exp3 <~ parseMin0Spaces)(code) match {
